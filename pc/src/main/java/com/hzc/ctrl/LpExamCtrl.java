...
//        start = true;
    ...
    private void showExamResult(Integer userId) {
        ...
        //作答时间
        long answerTime = getMinuteInterval(startTime, endTime);
    ...
    public void ajaxGetExamTime() {
        List<SysResource> resources = S.sysResourceService().getResources();
        Date date = new Date();
        long interval = Integer.MIN_VALUE;
        Map map = new HashMap<>();
        for (SysResource r : resources) {
            //正在考试中
            if (date.getTime() > r.getStartTime().getTime() && date.getTime() < r.getEndTime().getTime()) {
                ...
                if (date.getTime() < r.getStartTime().getTime()) {
                    //还未开始考试,并且后面有一场考试
                    long secondInterval = TimeUtil.getSecondInterval(date, r.getStartTime());
                    if (interval > secondInterval || interval < 0) {
                        //查询最近的一次考试
                        interval = secondInterval;
                        map.put("result", "false");
                        map.put("second", interval);
                    }
                } else {
                    //没有考试
